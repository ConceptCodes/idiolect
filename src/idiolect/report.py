...
from .models import Fingerprint, ComparisonResult, FINGERPRINT_AXES, AXIS_DESCRIPTIONS, AuthorType
...
def _render_score_dial(score: float, label: str, filepath: str):
    ...
    if len(theta_filled) > 0:
        ax.plot(theta_filled, [1]*len(theta_filled), color=color_hex, linewidth=10, solid_capstyle='round')

    ax.set_axis_off()
    ax.text(0, 0, f"{int(score)}", ha='center', va='center', fontsize=24, fontweight='bold', color=color_hex)
...
def generate_report(fingerprint: Fingerprint, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        pdf.cell(0, 12, "IDIOLECT — Linguistic Fingerprint Report", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        pdf.set_font("helvetica", "", 14)
        pdf.set_text_color(*COLOR_TEXT)
        pdf.cell(0, 8, f"Label: {fingerprint.label}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.cell(0, 8, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.cell(0, 6, f"Sentences: {fingerprint.sentence_count}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        if fingerprint.source_path:
            pdf.cell(0, 6, f"Source: {fingerprint.source_path}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.cell(60, 10, f"Confidence: {fingerprint.ai_confidence*100:.1f}%", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        pdf.set_font("helvetica", "", 10)
        expl = "AI indicators observed in text analysis."
        if fingerprint.ai_indicators:
            top_indicators = sorted(fingerprint.ai_indicators.items(), key=lambda x: abs(x[1]), reverse=True)[:3]
            expl = f"Key signals: {', '.join(f'{_humanize_feature(k)} ({v:.2f})' for k, v in top_indicators)}"
        ...
        pdf.image(str(radar_path), x=(210-100)/2, y=pdf.get_y(), w=100)
        ...
        if fingerprint.standout_traits:
            ...
            pdf.cell(0, 8, "No significant standout traits detected.", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def generate_comparison_report(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        pdf.image(str(radar_path), x=(210-120)/2, y=pdf.get_y()+5, w=120)
        ...
        pdf.set_font("helvetica", "", 11)
        pdf.cell(0, 6, f"Cosine Similarity: {result.cosine_similarity:.4f}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.cell(0, 6, f"Cosine Delta: {result.cosine_delta:.4f}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.cell(0, 6, f"Manhattan Delta (Burrows): {result.manhattan_delta:.4f}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        pdf.ln(5)
        pdf.set_font("helvetica", "B", 12)
        pdf.set_fill_color(*COLOR_NAVY)
        pdf.set_text_color(255, 255, 255)
        pdf.cell(0, 10, f" Same-Author Likelihood: {result.same_author_likelihood.replace('_', ' ').upper()}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
