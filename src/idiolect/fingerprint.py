...
# Used for z-score normalization and scaling.
POPULATION_STATS = {
...
def create_fingerprint(doc: Document, label: str) -> Fingerprint:
    ...
    lex_rare = _scale_0_100(flat.get("lexical.rare_word_ratio", 0.15), *POPULATION_STATS["lexical.rare_word_ratio"])
    lexical_richness = (lex_mattr + lex_mtld + lex_hdd + lex_rare) / 4
    
    # 2. Syntactic Complexity
    syn_len = _scale_0_100(flat.get("syntactic.sent_length_mean", 15.0), *POPULATION_STATS["syntactic.sent_length_mean"])
    syn_depth = _scale_0_100(flat.get("syntactic.parse_tree_depth_mean", 5.0), *POPULATION_STATS["syntactic.parse_tree_depth_mean"])
    syn_sub = _scale_0_100(flat.get("syntactic.subordinate_clause_ratio", 0.3), *POPULATION_STATS["syntactic.subordinate_clause_ratio"])
    syn_arc = _scale_0_100(flat.get("syntactic.dep_arc_length_mean", 2.5), *POPULATION_STATS["syntactic.dep_arc_length_mean"])
    ...
    epistemic_stance = _scale_0_100(flat.get("pragmatic.hedge_booster_ratio", 1.0), *POPULATION_STATS["pragmatic.hedge_booster_ratio"])
    
    # 5. Pacing Cadence
    pace_cv = _scale_0_100(flat.get("syntactic.sent_length_cv", 0.5), *POPULATION_STATS["syntactic.sent_length_cv"])
    pace_punct = _scale_0_100(flat.get("character.char_diversity", 0.6), *POPULATION_STATS["character.char_diversity"])
    pacing_cadence = (pace_cv + pace_punct) / 2
    
    # 6. Affective Intensity
    aff_std = _scale_0_100(flat.get("pragmatic.sentiment_std", 0.15), *POPULATION_STATS["pragmatic.sentiment_std"])
    aff_pos = _scale_0_100(flat.get("pragmatic.sentiment_positive_ratio", 0.3), *POPULATION_STATS["pragmatic.sentiment_positive_ratio"])
    aff_neg = _scale_0_100(flat.get("pragmatic.sentiment_negative_ratio", 0.2), *POPULATION_STATS["pragmatic.sentiment_negative_ratio"])
    affective_intensity = (aff_std + aff_pos + aff_neg) / 3
    
    # 7. Interactive Engagement
    int_pron = _scale_0_100(flat.get("pragmatic.pron_second", 12.0), *POPULATION_STATS["pragmatic.pron_second"])
    int_disc = _scale_0_100(flat.get("pragmatic.discourse_marker_ratio", 10.0), *POPULATION_STATS["pragmatic.discourse_marker_ratio"])
    int_quest = _scale_0_100(flat.get("character.question_sentence_ratio", 0.06), *POPULATION_STATS["character.question_sentence_ratio"])
