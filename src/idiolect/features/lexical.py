...
def extract_lexical(doc: Document) -> dict[str, float]:
    ...
    try:
        ...
        features["mattr"] = float(lex.mattr(window_size=min(50, N)))
        features["mtld"] = float(lex.mtld(threshold=0.72))
        if N >= 42: # HD-D usually requires at least 42 words
    ...
    features["hapax_legomena_ratio"] = v1 / N if N > 0 else 0.0
    features["hapax_dislegomena_ratio"] = v2 / N if N > 0 else 0.0
    ...
    features["yule_k"] = 10000.0 * (sum_i2_vi - N) / (N ** 2) if N > 0 else 0.0
    ...
    valid_zipfs = [z for z in zipfs if z > 0] # Filter out words not in wordfreq dictionary
    if valid_zipfs:
        features["avg_zipf_frequency"] = float(np.mean(valid_zipfs))
        features["rare_word_ratio"] = sum(1 for z in valid_zipfs if z < 3.0) / len(valid_zipfs)
        features["common_word_ratio"] = sum(1 for z in valid_zipfs if z > 5.0) / len(valid_zipfs)
