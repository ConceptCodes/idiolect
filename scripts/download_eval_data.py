...
def download_federalist_benchmark() -> Path:
    ...
    for i, match in enumerate(matches):
        ...
        (out_dir / f"{author}_essay_{essay_num:02d}.txt").write_text(
            essay_content, encoding="utf-8"
        )

    print(
        f"✓ Saved complete Federalist essays: {h_count} Hamilton, "
        f"{m_count} Madison, {j_count} Jay to {out_dir}"
    )
    return out_dir
