...
def evaluate_ai_detection(max_samples: int = 25):
    ...
    if not ai_dir.exists():
        console.print(
            "[bold red]Dataset not found![/] Run: python scripts/download_eval_data.py ai"
        )
        return
    ...
    console.print(
        Panel(
            f"Evaluating AI Detection on [bold cyan]{len(human_files)} Human[/] "
            f"and [bold cyan]{len(ai_files)} AI[/] texts from HC3...",
    ...
    table.add_row(
        "Precision", f"{precision * 100:.1f}%", "When flagged as AI, probability it is AI"
    )
    ...
    table.add_row(
        "False Accusation Rate (FPR)", f"{fpr * 100:.1f}%", "Human papers wrongly flagged as AI"
    )
    table.add_row(
        "Uncertain Rate",
        f"{(uncertain / total) * 100:.1f}%",
        "Cases flagged for human teacher review",
    )
    table.add_row(
        "Throughput",
        f"{total / duration:.1f} docs/sec",
        f"Processed {total} docs in {duration:.1f}s",
    )
...
def evaluate_literary_authorship(train_samples: int = 5, test_samples: int = 3):
    ...
    if not auth_dir.exists():
        console.print(
            "[bold red]Literary dataset not found![/] "
            "Run: python scripts/download_eval_data.py authors"
        )
        return
    ...
    profiles = {"Conan Doyle": p_doyle, "Jane Austen": p_austen, "Mark Twain": p_twain}
    ...
    console.print(
        f"\n[bold green]Literary Attribution Accuracy:[/] "
        f"[bold]{correct}/{total} ({acc_pct:.1f}%)[/]\n"
    )
...
def evaluate_federalist_attribution(holdout_per_author: int = 5):
    ...
    if not fed_dir.exists():
        console.print(
            "[bold red]Federalist dataset not found![/] "
            "Run: python scripts/download_eval_data.py federalist"
        )
        return
    ...
    console.print(
        Panel(
            "Evaluating Historical Authorship Attribution on The Federalist Papers:\n"
            f"Hamilton ({len(hamilton_files)} full essays) vs. "
            f"Madison ({len(madison_files)} full essays)\n"
            "Composite baseline profiles generated from training corpora.",
    ...
    console.print(table)
    fed_acc_pct = (correct / total) * 100
    console.print(
        f"\n[bold green]Federalist Attribution Accuracy:[/] "
        f"[bold]{correct}/{total} ({fed_acc_pct:.1f}%)[/]\n"
    )
